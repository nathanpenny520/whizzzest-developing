<template>
  <nav class="navbar" role="navigation">
    <div class="nav-inner">
      ...
      <div class="nav-right">
        <LanguageSwitcher />
        <button v-if="!authStore.isLoggedIn" @click="handleLogin" class="login-btn">
          {{ t('common.login') }}
        </button>
      ...
      <div v-if="mobileOpen" class="mobile-menu">
        <div v-for="cluster in clusters" :key="cluster.key" class="mobile-cluster">
          ...
          <span
            ...
            :class="['mobile-item', { disabled: !item.path && (item as any).action !== 'chat' }]"
          >
            {{ item.label }}
...
function handleMobileItem(item: ClusterItem) {
  if (item.action === 'chat') {
    mobileOpen.value = false
    window.__huaNuoOpen?.()
    return
...
function handleLogin() {
  authStore.requireLogin(() => {}, 'default')
}
