  ...
  <div class="min-h-screen bg-gray-50">
    ...
    >
      <div class="absolute inset-0 z-0">
        <picture>
          <source srcset="/optimized/commerce.webp" type="image/webp" />
          <img
    ...
    <section class="py-16 bg-white">
      <div class="container mx-auto px-4">
        <h2 class="text-3xl font-bold text-center mb-12 text-red-600">
          {{ t('merchant.sections.featured.title') }}
        </h2>
        <div class="space-y-8">
          <div
            ...
            <div class="relative">
              <div class="grid grid-cols-3 gap-0">
                <template v-if="merchant.images">
                  <picture v-for="(img, idx) in merchant.images" :key="idx">
                    <source v-if="webpSrc(img)" :srcset="webpSrc(img)!" type="image/webp" />
                    <img
                      :src="img"
                      :alt="merchant.name"
                ...
                <template v-else>
                  <picture>
                    <source
                      v-if="webpSrc(merchant.logo)"
                      :srcset="webpSrc(merchant.logo)!"
                      type="image/webp"
                    />
                    <img
                      :src="merchant.logo"
                  ...
                  <picture>
                    <source
                      v-if="webpSrc(merchant.image)"
                      :srcset="webpSrc(merchant.image)!"
                      type="image/webp"
                    />
                    <img
                      :src="merchant.image"
                  ...
                  <picture>
                    <source v-if="webpSrc(teamImg)" :srcset="webpSrc(teamImg)!" type="image/webp" />
                    <img
                      :src="teamImg"
                      :alt="merchant.name"
                      class="w-full h-64 object-cover"
              ...
              <div
                class="absolute top-3 right-3 bg-red-600 text-white px-3 py-1 rounded-full text-sm font-medium"
              >
                {{ merchant.category }}
              </div>
            </div>
            <div class="p-6">
              <div class="flex items-center justify-between mb-3">
                <a
                  ...
                  class="text-xl font-bold text-gray-800 hover:text-red-600 transition-colors"
                  >{{ merchant.name }}</a
                >
                ...
                <div class="flex items-center text-yellow-500">
                  <svg
                    ...
                    :key="i"
                    class="w-4 h-4"
                    :class="i <= merchant.rating ? 'text-yellow-500' : 'text-gray-300'"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
                    <path
              ...
              <div class="mb-4">
                <h4 class="text-sm font-semibold text-gray-700 mb-2">
                  {{ t('merchant.sections.featured.products') }}
                </h4>
              ...
              <div class="mb-4">
                <h4 class="text-sm font-semibold text-gray-700 mb-2">
                  {{ t('merchant.sections.featured.advantages') }}
                </h4>
                <ul class="text-sm text-gray-600 space-y-1">
                  <li
                    v-for="(advantage, index) in merchant.advantages.slice(0, 2)"
                    :key="index"
                    class="flex items-start"
                  >
                    <svg
                      class="w-4 h-4 text-green-500 mr-1 mt-0.5 flex-shrink-0"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                      <path
              ...
              <div class="border-t pt-4">
                <h4 class="text-sm font-semibold text-gray-700 mb-2">
                  {{ t('merchant.sections.featured.contact') }}
                </h4>
                <div class="space-y-1 text-sm text-gray-600">
                  ...
                  <div v-if="merchant.weixin" class="mt-3">
                    ...
                    <picture>
                      <source
                        v-if="webpSrc(merchant.weixin)"
                        :srcset="webpSrc(merchant.weixin)!"
                        type="image/webp"
                      />
                      <img
                        :src="merchant.weixin"
    ...
    <section
      class="py-16"
      :style="{
        backgroundImage: `url(${commentBackgroundImg})`,
    ...
    >
      <div class="container mx-auto px-4">
        <h2 class="text-3xl font-bold text-center mb-12 text-red-600">
          {{ t('merchant.sections.reviews.title') }}
        </h2>
        ...
        <div class="relative max-w-4xl mx-auto">
          <div class="overflow-hidden rounded-lg">
            ...
            >
              <div v-for="review in reviews" :key="review.id" class="w-full flex-shrink-0">
                <div class="bg-white/80 backdrop-blur-sm rounded-lg p-8 md:p-10 shadow-md mx-4">
                  <div class="flex items-center mb-6">
                    <div
                      class="w-14 h-14 rounded-full bg-red-100 flex items-center justify-center text-red-600 font-bold text-xl"
                    >
          ...
          <div class="flex justify-center mt-8 space-x-2">
            <button
              ...
              :class="
                currentSlide === index ? 'bg-red-600/70 w-8' : 'bg-gray-300/50 hover:bg-gray-400/70'
              "
    ...
    <section v-if="coupons.length > 0" class="coupon-section">
      ...
      <div class="coupon-grid">
        <CouponCard
          ...
          :key="c.id"
          :coupon="c"
          :pre-claimed="claimedIds.has(c.id)"
        />
      </div>
    </section>
  </div>
...
// WebP name lookup for imported images (maps Vite-resolved URL to WebP stem)
const webpMap: Record<string, string> = {}
webpMap[wanzaihuapaoImg] = 'changfang'
webpMap[huapaoqingImg] = 'hezuoqiye'
webpMap[teamImg] = 'team'
webpMap[qnspImg1] = 'qnsp (1)'
webpMap[qnspImg2] = 'qnsp (2)'
webpMap[qnspImg3] = 'qnsp (3)'
webpMap[yzxfBswzImg] = 'yzxf_bswz'
webpMap[commentBackgroundImg] = 'comment-backgroud'
webpMap[tailinLogoImg] = 'tailin-logo'
webpMap[tailinWenziImg] = 'tailin-wenzi'
webpMap[tailinWeixinImg] = 'tailin-accountweixin'
function webpSrc(imgUrl: string): string | null {
  const name = webpMap[imgUrl]
...
const merchantImages: MerchantImage[] = [
  ...
  { id: 2, logo: huapaoqingImg, image: yzxfBswzImg, images: [qnspImg1, qnspImg2, qnspImg3] },
  {
    id: 3,
    logo: tailinLogoImg,
    image: tailinWenziImg,
    images: [tailinLogoImg, tailinWenziImg, tailinGongchangImg],
    weixin: tailinWeixinImg,
  },
]
...
const merchants = computed(() => {
  return merchantImages.map((img, index) => {
    ...
    return {
      ...
      website:
        index === 0
          ? 'https://wwbnn.lanzouu.com/i5Esy3j90hcb'
          : index === 1
            ? 'http://www.qnsp.cn/'
            : 'http://www.wztlhp.com/',
      weixin: img.weixin,
